...
# Using a fixed seed for reproducibility - change this if you want different shuffles
random.seed(42)
...
def _limit_dataset(dataset: list[dict], max_samples: int | None) -> list[dict]:
    ...
    # Shuffling before slicing so we get a representative subset rather than
    # always taking the first N examples (which can be biased toward easier ones).
