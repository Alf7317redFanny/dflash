...
class DFlashAttention(nn.Module):
    ...
    def __call__(self, x, x_ctx, rope, cache):
        ...
        S = x_ctx.shape[1]
        if self.is_sliding:
            # Keep only the last (sliding_window - 1) context tokens so the
            # current query token fits within the window. Without this trim
            # the attention mask would cover more tokens than the window allows.
